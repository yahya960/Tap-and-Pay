...
public class SDKHelper {
    ...
    public interface InitCPSSDKCallback {
        void doAction() throws MGConfigurationException;
    }
    ...
    public static void initCPSSDK(final Context context, final InitCPSSDKCallback initCPSSDKCallback, final boolean isUIUpdateNeeded) {
        ...
        customConfiguration = new CustomConfiguration.Builder()
                .domesticCurrencyCode(978)
                .keyValidityPeriod(45)
                .maxConsecutivePaymentsForLVT(10)
                .maxCumulativeAmountForLVT(100)
                .singleTransactionAmountLimitForLVT(50)
        ...
        SDKInitializer.INSTANCE.initialize(context, sdkControllerListener,
                NotificationUtil.getNotification(context,
                        ...
                        context.getString(R.string.cps_notification_channel_id)), customConfiguration);
    }
            ...
            public void onError(SDKError<SDKInitializeErrorCode> initializeError) {
                ...
                if (initializeError.getErrorCode() == SDKInitializeErrorCode.SDK_INITIALIZED) {
                    ...
                    //Ignore as initialization is happening elsewhere
                        ...
                        SDKInitializeErrorCode.DEVICE_SUSPICIOUS == initializeError.getErrorCode()) {
                    ...
                    if(SDKInitializeErrorCode.DEVICE_SUSPICIOUS == initializeError.getErrorCode()) {
                        new Handler(Looper.getMainLooper()).post(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(context, "Device Suspicious",
                                        Toast.LENGTH_SHORT).show();
                            }
                        });
                        SharedPreferences sp = context.getApplicationContext().getSharedPreferences("firstRun", Activity.MODE_PRIVATE);
    ...
    public static void performWalletSecureEnrollmentFlow(final Context context) {
        ...
        if (state == WalletSecureEnrollmentState.WSE_REQUIRED) {
            ...
            wseService.startWalletSecureEnrollment(new WalletSecureEnrollmentListener() {
                ...
                public void onProgressUpdate(WalletSecureEnrollmentState wseState) {
                    ...
                    if (wseState == WalletSecureEnrollmentState.WSE_COMPLETED) {
                        ...
                        Toast.makeText(context, "performWalletSecureEnrollmentFlow - WSE_COMPLETED",
                                Toast.LENGTH_SHORT).show();
                        broadcastInitWseCompleted(context);
                    }
                }
                ...
                public void onError(WalletSecureEnrollmentError wbDynamicKeyRenewalServiceError) {
                    ...
                    if(wbDynamicKeyRenewalServiceError.getSdkErrorCode() == WalletSecureEnrollmentErrorCodes.DEVICE_SUSPICIOUS){
                        Toast.makeText(context, "performWalletSecureEnrollmentFlow - onError: " + "Device Suspicious",
                                Toast.LENGTH_SHORT).show();
                    }else {
        ...
        } else if (state == WalletSecureEnrollmentState.WSE_NOT_REQUIRED) {
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(context, "performWalletSecureEnrollmentFlow: " + state,
                            Toast.LENGTH_SHORT).show();
                    broadcastInitComplete(context, false);
                }
            });
        } else if (state == WalletSecureEnrollmentState.WSE_COMPLETED) {
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(context, "performWalletSecureEnrollmentFlow: " + state,
    ...
    private static void broadcastInitWseCompleted(Context context) {
        Intent wseDone = new Intent(AppConstants.ACTION_RELOAD_CARDS);
        LocalBroadcastManager.getInstance(context.getApplicationContext()).sendBroadcast(wseDone);
    }
