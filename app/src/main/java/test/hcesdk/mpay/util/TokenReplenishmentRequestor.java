        ...
        AppLogger.i(TAG, "Initiating Replenishment for tokenizedCardId: " + tokenizedCardId);
            ...
            provBs.sendRequestForReplenishment(tokenizedCardId, new PushServiceListener() {
                ...
                public void onError(ProvisioningServiceError provisioningServiceError) {
                    ...
                    String toastMessage="Replenish Failed ";
                    if(provisioningServiceError.getSdkErrorCode() == ProvisioningServiceErrorCodes.REPLENISHMENT_PERMANENT_ERROR) {
                        toastMessage = "Replenish Blocked " + ":" + provisioningServiceError.getErrorMessage();
                    } else {
                        toastMessage = toastMessage + ":" + provisioningServiceError.getErrorMessage();
                    }

                    Toast.makeText(AndroidContextResolver.getApplicationContext(),toastMessage,Toast.LENGTH_LONG).show();
                }
