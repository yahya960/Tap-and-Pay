...
public class MainActivity extends AppCompatActivity implements CardListAdapter.CardListAdapterCallback{
    ...
    private SecureKeypadBuilder mKeypadModel;
...
//                inputer.finish();
    ...
    private void changePin() {
        WalletPinManager.getInstance().bindAbstractWalletPinService(new AbstractWalletPinService() {
            @Override
            public void onSetWalletPin(final CHCodeVerifier chCodeVerifier) {
                key_pad_fragment.setVisibility(View.VISIBLE);
                fabAdd.hide();
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        setupGSK_43(chCodeVerifier, SecureInputType.CHANGE_PIN);
                    }
                }, 1);
            }
            ...
            public void onVerifyWalletPin(CHCodeVerifier chCodeVerifier) {
                ...
                fabAdd.hide();
                //To use secure keypad provided use this
                setupGSK_43(chCodeVerifier, SecureInputType.VERIFY_PIN);
            }
        ...
        try {
            AppLogger.i(TAG, "invokeChangeWalletPin");
            WalletPinManager.getInstance().invokeChangeWalletPin();
        } catch (WalletPinException e) {
            e.printStackTrace();
        }
    }
    ...
    WalletPinEventListener walletPinEventListener = new WalletPinEventListener() {
        ...
        public void onPinSet() {
            key_pad_fragment.setVisibility(View.GONE);
            fabAdd.show();
            toggleProgress(true);
            loadCards();
        ...
        public void onPinVerified() {
            ...
            findViewById(R.id.key_pad_fragment).invalidate();
        }
    ...
    //set up PIN Pad
    private void setupGSK_43(CHCodeVerifier chCodeVerifier, final SecureInputType secureInputType) {

        mKeypadModel = Util.decorateKeyPadWithDefaultSettings(chCodeVerifier, getApplicationContext());
        final boolean isConfirmPwd;
        // Set PIN Labels according to SecureInputType
        switch (secureInputType) {
            case SET_PIN:
                mKeypadModel.setTextLabel(getString(R.string.label_setPin));
                mKeypadModel.setConfirmationTextLabel(getString(R.string.label_setPin_confirm));
                isConfirmPwd = true;
                break;
            case VERIFY_PIN:
                mKeypadModel.setTextLabel(getString(R.string.label_verifyPin));
                isConfirmPwd = false;
                break;
            case CHANGE_PIN:
                mKeypadModel.setTextLabel(getString(R.string.label_changePin));
                mKeypadModel.setConfirmationTextLabel(getString(R.string.label_changePin_confirm));
                isConfirmPwd = true;
                break;
            default:
                isConfirmPwd = false;
        ...
        Fragment fragment = mKeypadModel.build(false, isConfirmPwd, false, new SecureKeypadEventsListener() {
            @Override
            ...
            public void textFieldSelected(int i) {
                if (isConfirmPwd) {
                    if (i == 0) {
                        mKeypadModel.setOkButtonText("NEXT");
                    } else if (i == 1) {
                        mKeypadModel.setOkButtonText("OK");
                    }
                    AppLogger.i(TAG, "textFieldSelected");
                }
            }
        });
        mKeypadModel.setCharacterArray("1234567890");
        getSupportFragmentManager().beginTransaction().replace(R.id.key_pad_fragment, fragment).commit();
    }
    ...
    public void onPaymentClicked(MyDigitalCard card) {
        ...
        startActivity(new Intent(this, PaymentQRActivity.class));
    }
    ...
    public void onDsrpClicked(MyDigitalCard card) {
        ...
        if(!card.isDefaultCardFlag()) {
            DigitalizedCardManager.getDigitalizedCard(card.getTokenId()).setDefault(PaymentType.CONTACTLESS, new AbstractAsyncHandler<Void>() {
                ...
                public void onComplete(AsyncResult<Void> asyncResult) {
                    if (asyncResult.isSuccessful()) {
                        AppLogger.i(TAG, "setDefault called from onDsrpClicked :: Listener");
                        toggleProgress(false);
                        Intent dsrp = new Intent(MainActivity.this, DsrpActivity.class);
                        startActivity(dsrp);
                    } else {
                        Snackbar snackbar = Snackbar.make(coordinatorLayout, "Card could not be set as default card:\n Error message::" + asyncResult.getErrorMessage(), Snackbar.LENGTH_SHORT);
                        snackbar.show();
    ...
    private void showSettingsDialog() {
        String[] settingsList = {getString(R.string.action_changePin), getString(R.string.action_wipe_data), getString(R.string.change_benchmarking),getString(R.string.payment_experience)};
        int selected = 0;
        ...
        DialogHelper.createBenchmarkingDialog(MainActivity.this, getString(R.string.action_settings), settingsList, selected, (dialog, i) -> {
            ...
            switch (i) {
                case 0:
                    Toast.makeText(getApplicationContext(), "Change PIN Action Clicked", Toast.LENGTH_SHORT).show();
                    changePin();
                    break;
