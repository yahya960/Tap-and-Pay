...
public class PaymentDSRPActivity extends AppCompatActivity implements RemotePaymentServiceListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        performMCDSRPCryptogram(amount, currencyCode, countryCode, transactionType1, cryptogramDataType,tokenId);
    }
    ...
    private void performMCDSRPCryptogram(long amount, char currencyCode, char countryCode, TransactionType transactionType, CryptogramDataType cryptogramDataType,String tokenId) {
        toggleProgress(true);
        ...
        try {
            PaymentInputData paymentInputData = new PaymentInputData.PaymentInputBuilder(PaymentType.DSRP)
                    ...
                    .build();
            DigitalizedCard card = DigitalizedCardManager.getDigitalizedCard(tokenId);
            if (card.isDefault(PaymentType.DSRP,null).waitToComplete().getResult()){
                PaymentBusinessManager.getPaymentBusinessService().generateApplicationCryptogram(PaymentType.DSRP,
                        paymentInputData, this);
            }else{
                PaymentBusinessManager.getPaymentBusinessService().generateApplicationCryptogram(card,PaymentType.DSRP,
                        paymentInputData, this);
            }
    ...
    public void onAuthenticationRequired(PaymentService paymentService, CHVerificationMethod chVerificationMethod, long cvmResetTimeout) {
        ...
        if (chVerificationMethod == CHVerificationMethod.BIOMETRICS
            ...
            if(delegatedCDCVM) {
                verifier = (DeviceCVMVerifier) paymentService.getCHVerifier(chVerificationMethod);
    ...
    private void verifyTransactionStatus(boolean isSuccess, long successCurrentTS) {
        if(isSuccess) {
            ...
            Toast toast = Toast.makeText(this, "Transaction authentication failed", Toast.LENGTH_LONG);
