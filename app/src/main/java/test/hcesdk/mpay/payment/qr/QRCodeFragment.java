...
public class QRCodeFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        Toast.makeText(getActivity(), "Selected QR type: " + qrType, Toast.LENGTH_LONG).show();
        setQRImage();
//       sendReplenishmentRequestIfNeeded();
    ...
    private void setQRImage() {
        ...
        String qrDataBase64 = "";
        payloadFormatValue = "4350563031";
        if (qrType.equals("Standard")) {
            qrDataBase64 = Base64.encodeToString(Util.hexStringToByteArray(buildQrTlvStandard(qrCodeData, transactionContext)), Base64.DEFAULT);
        } else {
            qrDataBase64 = Base64.encodeToString(Util.hexStringToByteArray(buildQrTlvCondensed(qrCodeData)), Base64.DEFAULT);
        }
        AppLogger.d(TAG, "Base64StringCondensed=" + qrDataBase64);
    ...
    private String buildQrTlvStandard(QRCodeData qrCodeData, TransactionContext transactionContext) {
        ...
        byte[] tag85 = new BerTlvBuilder()
                .addHex(new BerTag(0x85), payloadFormatValue) //CPV01
                .buildArray();
    ...
    /**
     * buildQrTlvCondensed for constructTLV data
     */
    private String buildQrTlvCondensed(QRCodeData qrCodeData) {
        String tag85Str = null;
        String template61Str1 = null;
        String template63str1 = null;
        String template63str2 = null;
        ...
        // build payload formater
        byte[] tag85 = new BerTlvBuilder()
                .addHex(new BerTag(0x85), payloadFormatValue)
                .buildArray();
        tag85Str = HexUtil.toHexString(tag85);
        ...
        if (qrCodeData.getCardMainAppTemplate() != null) {
            // build template 61 for application 1
            byte[] tag63 = new BerTlvBuilder()
                    .addHex(new BerTag(0x63), HexUtil.toHexString(qrCodeData.getCondensedPaymentData()))
                    .buildArray();
            template63str1 = HexUtil.toHexString(tag63);
            byte[] tag61 = new BerTlvBuilder()
                    .addHex(new BerTag(0x61), HexUtil.toHexString(qrCodeData.getCardMainAppTemplate()) + tagC5str + template63str1)
        ...
        if (qrCodeData.getCardAliasAppTemplate() != null) {
            // build template 61 for application 2
            byte[] tag63_2 = new BerTlvBuilder()
                    .addHex(new BerTag(0x63), HexUtil.toHexString(qrCodeData.getCondensedPaymentData()))
                    .buildArray();
            template63str2 = HexUtil.toHexString(tag63_2);
            byte[] tag61_2 = new BerTlvBuilder()
                    .addHex(new BerTag(0x61), HexUtil.toHexString(qrCodeData.getCardAliasAppTemplate()) + tagC5str + template63str2)
                    .buildArray();
            template61Str2 = HexUtil.toHexString(tag61_2);
        } else {
            template61Str2 = "";
        }
